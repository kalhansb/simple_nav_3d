// Moved comments: doc/simple_nav_planner_node_notes.md
#include <chrono>
#include <cmath>
#include <limits>
#include <memory>
...
// Keeps the global path's waypoints inside the local map, from the one nearest
// the robot up to the first outside; the last kept one becomes local_target.
// Returns false if none is kept. (notes: planner-slice-global-path)
...
// Marks every cell outside a corridor of half-width radius_m around the
// centerline occupied, keeping the local A* in global's homotopy. Never frees
// an occupied cell; the robot position is always inside the corridor.
// (notes: planner-corridor-mask)
...
public:
  ...
  : Node("simple_nav_planner")
  {
    ...
    // Local role only: the global path picks the local target and the corridor
    // mask. Without a global path the local planner plans to the goal on the
    // unmasked map. (notes: planner-local-follows-global-path)
    if (is_local_role_) {
      ...
      [this](const geometry_msgs::msg::PoseStamped::SharedPtr msg) {
        if (has_goal_) {
          ...
          if (d_goal > goal_change_reset_m) {
            ...
            // Drop the global path too: it was planned to the old goal, and the
            // global planner publishes nothing when it fails, so the local
            // planner would keep masking toward the old goal.
            // (notes: planner-drop-global-on-goal-change)
    ...
    // Logged on liveness_logger_, not get_logger(): the launch sets this node's
    // logger to warn, which would hide this banner naming the watched map
    // topic. It pairs with the periodic global plan ok heartbeat.
    // (notes: planner-startup-banner-logger)
    RCLCPP_INFO(
      liveness_logger_,
...
private:
  ...
  /// Clears the goal and plan cache and publishes an empty path. anomalous
  /// selects WARN: this node logs at WARN, so an anomalous clear (navigator
  /// stopped sending goals) at INFO would leave no trace.
  /// (notes: planner-clear-plan-severity)
  ...
  void on_tick()
  {
    ...
    if (!has_goal_ || !has_odom_ || !map_ready) {
      ...
      // Warn when a goal is pending but odom or map is missing, after a
      // kStarveGraceTicks grace (dscovox sends no map until its first fused
      // frame). has_goal_ gates it: no goal is idle by design.
      // (notes: planner-starvation-warning)
      if (has_goal_ && (!has_odom_ || !map_ready)) {
        ++ticks_starved_;
        if (ticks_starved_ > kStarveGraceTicks) {
        ...
        // No goal pending: idle, not starving. Clear starvation here, since the
        // path below needs all three inputs; otherwise a goal gap would later
        // be reported as an input recovery.
        // (notes: planner-starvation-clear-when-idle)
        clear_starvation();
      }
      return;
    }
    ...
    // Global UGV role replans at most every ugv_global_replan_period_sec,
    // stamped per attempt, not per success, so failing plans are limited too. A
    // goal change (force_replan_) or an invalidated latched path preempts it.
    // (notes: planner-global-replan-decimation)
    ...
    // Local role: target the global path's exit from the local window, with the
    // map masked to a corridor of ugv_local_corridor_radius_m around it. If
    // corridor A* fails, retry on the unmasked map.
    // (notes: planner-local-homotopy-refine)
    ...
    if (out.has_path) {
      ...
      // Side-flip rejection applies to the global UGV planner only; on the
      // local planner's smaller, faster window the cooldown would only lock in
      // stale paths. (notes: planner-side-flip-global-only)
      ...
      // Append the real goal when the path end is within (kGoalSnapEpsM,
      // kGoalSnapMaxM] of it and the straight run is free. UGV only
      // (segment_free is 2D). Test on the unmasked map_snapshot, never
      // map_for_planner. (notes: planner-goal-snap-append)
      ...
      // Stamp the goal orientation onto the last waypoint only when it lies
      // within 2x final_goal_tolerance of the goal; the local path ends at an
      // intermediate target. After an append dg is 0, so the tip is stamped.
      // (notes: planner-stamp-goal-orientation)
      ...
      // Positive heartbeat for the global instance, printed only after a real
      // plan, so a live global planner is checkable from the logs rather than
      // inferred from silence. (notes: planner-global-plan-heartbeat)
      if (is_global_ugv) {
      ...
      // Log both goal-snap counters on both roles; the blocked count is the
      // only evidence the segment test does work. role= is needed because both
      // instances share the nav_liveness logger name.
      // (notes: planner-goal-snap-counters)
  ...
  // Liveness lines use a logger outside this node's own subtree, which the
  // launch file sets to warn, so the INFO heartbeat survives. Do not escalate
  // them to WARN instead: that stream feeds the failure counts.
  // (notes: planner-liveness-logger)
  rclcpp::Logger liveness_logger_{rclcpp::get_logger("nav_liveness")};

  // Replan decimation state. last_plan_attempt_ is stamped on every attempt,
  // not every success, so a failing planner is rate-limited too.
  // (notes: planner-replan-attempt-stamp)
